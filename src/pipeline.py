...
def create_db(tickets,events,mart):
 ...
 con.executemany('INSERT INTO fact_assignment(ticket_key,queue_key,event_sequence,assigned_at,unassigned_at) VALUES(?,?,?,?,?)',[(ticket_keys[r['ticket_id']],keys['queue'][r['queue']],r['sequence'],r['assigned_at'],r['unassigned_at'] or None) for r in events]); con.commit(); counts={t:con.execute(f'SELECT count(*) FROM {t}').fetchone()[0] for t in ('stg_tickets','stg_assignment_history','fact_ticket_sla','fact_assignment')}; con.close(); return counts
